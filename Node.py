...
class Node:
    ...
    def _tx_check_input_correct(self, tx: Transaction):
        ...
        for input in tx.input_list:
            ...
            key = sha256((str(input_number.tx_number) + str(input_output_pubkey) + str(input_output_value)).encode()).hexdigest()
            if (not self.blockchain.UTXO.has_key(key)):
                return False
            
            # check if each output in the input has the same public key, and that key can verify the signature on this transaction
            target_tx = curr.currBlock.tx
            if first_pubkey != input_output_pubkey:
                return False
            signed = target_tx.sig.encode()
            sender_pubkey = VerifyKey(input_output_pubkey.encode(), encoder=Base64Encoder)
            try:
                sender_pubkey.verify(signed, encoder=Base64Encoder)
                print(f"Transaction {target_tx.tx_number} is valid")
                
            except nacl.exceptions.BadSignatureError:
                print(f"Transaction {target_tx.tx_number} is invalid")
                return False
    ...
    def mine_block(self, tx: Transaction):
        if self.check_valid_tx(tx):
            ...
            prevHashing = prevBlock.currBlock.get_hash()
            nonce = 0 # TODO
